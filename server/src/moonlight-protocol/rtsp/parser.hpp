 ...
 */
// `inline` added vs the reference copy so this header can be #included in more
// than one TU of the same binary (the RTSP server + the KAT test) without a
// multiple-definition link error. Body is otherwise verbatim from Wolf.
inline std::optional<RTSP_PACKET> parse(std::string_view msg) {
 ...
 */
inline std::string to_string(const RTSP_PACKET &pkt) {
